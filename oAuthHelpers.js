...
class OAuthHelpers {
    ...
    static async createEvent(context, meetingDetails) {
        if (!context) {
            throw new Error('OAuthHelpers.sendMail(): `context` cannot be undefined.');
        }
        if (!meetingDetails) {
            throw new Error('OAuthHelpers.sendMail(): `meetingDetails` cannot be undefined.');
        }

        const client = new SimpleGraphClient(meetingDetails.tokenResponse.token);
        const me = await client.getMe();

        await client.createEvent(meetingDetails);
        //await context.sendActivity(`I created an event.`);
    }
